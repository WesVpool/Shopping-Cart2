  ...
  let list = items.map((item, index) => {
    ...
    return (
      <li key={index}>
        ...
        <Button variant="primary" size="large" name={item.name} type="submit" onClick={addToCart}>
  ...
  return (
    <Container>
      <Row>
        ...
        <Col>
          <h1>CheckOut </h1>
          <Button onClick={pay}>Check Out $ {finalList().total}</Button>
          <div> {finalList().total > 0 && finalList().final} </div>
        </Col>
      </Row>
      <Row>
      <Button style={{width: "500px"}} onClick={restockProducts}>Restock</Button>
      </Row>
    </Container>
  );
};
